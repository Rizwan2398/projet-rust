...
pub fn robot_movement_system(
    ...
    let tile_positions: Vec<((i32, i32), TileType)> = param_set.p1()
        .iter()
        .map(|(tile, transform)| {
    ...
    for (mut transform, mut position, _robot_type, robot_state) in param_set.p0().iter_mut() {
        if rng.gen_bool(0.02) {
            let (dx, dy) = if *robot_state == RobotState::Exploring {
                ...
                let dx = if position.x > 0 { -1 } else if position.x < 0 { 1 } else { 0 };
                let dy = if position.y > 0 { -1 } else if position.y < 0 { 1 } else { 0 };
                (dx, dy)
            };
